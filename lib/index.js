...
Ramen.prototype.page = function() {
  privateGo({ integration: this, page: arguments });
};

Ramen.prototype.track = function(track) {
  // If there has not been an identify call yet, do nothing.
  if (!this.identified) {
    return;
  }

  window.Ramen.Api.track_named(track.event());
};
...
function privateGo(options) {
  ...
  email = email || integration.analytics.user().email;
  id = id || integration.analytics.user().id();

  window.ryan = [integration.analytics, integration.analytics.user(), email, id, options];
